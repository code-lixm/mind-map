...
class Render {
  ...
  bindEvent() {
    ...
    this.handlePasteEvent = (e) => {
      ...
      const isInTextEdit = this.textEdit && this.textEdit.showTextEdit
  ...
  render(callback, source) {
    ...
    // 如果正在渲染，只标记需要重渲染，由渲染完成回调处理
    if (this.isRendering) {
      this.needRerender = true
      return
    }
    // 使用 RAF 调度，天然合并同一帧内的所有调用
    if (this.rafId) return // 已经调度过了
  ...
  // 真正的渲染
  _render() {
    // 双重保险：RAF 回调时再检查一次（理论上不会进入，但防御性编程）
    if (this.isRendering) {
      this.needRerender = true
      return
    }
  ...
  clearActiveNodeList() {
    ...
    this.mindMap.command.startBatch()
    const list = [...this.activeNodeList]
    this.activeNodeList = []
    list.forEach(node => {
      this.mindMap.execCommand('SET_NODE_ACTIVE', node, false)
      this.emitNodeInactiveEvent(node, [])
    })
  ...
  // 批量添加节点到激活列表里
  addNodesToActiveList(nodes) {
    if (nodes.length <= 0) return

    // 触发一次 before_node_active 事件，确保 TextEdit 等组件能正常响应
    // 只需要触发一次，避免 O(N^2) 开销
    const cleanedActiveNodeList = this.cleanNodeListForEvent(this.activeNodeList)
    this.mindMap.emit('before_node_active', nodes[0], cleanedActiveNodeList)
  ...
  // 批量从激活列表里移除节点
  removeNodesFromActiveList(nodes) {
    this.mindMap.command.startBatch()
    const uidSet = new Set(nodes.map(node => node.getData('uid')))
    // 过滤出新的激活列表
  ...
  selectAll() {
    ...
    this.addNodesToActiveList(nodes)
    this.emitNodeActiveEvent()
  }
  ...
  handlePaste(event) {
    ...
    const clipboardData =
      event.clipboardData || event.originalEvent?.clipboardData || window.clipboardData

    if (!clipboardData) {
      // 如果没有 clipboardData，尝试使用内部缓存数据
      this.paste()
      return
    }
    ...
    if (items && items.length > 0) {
      ...
      if (textItem) {
        textItem.getAsString(str => {
          text = str
          this.handlePasteData({ text, img })
        })
      }
    } else {
      text = clipboardData.getData('text') || clipboardData.getData('Text')
    }

    // 如果没有文本（只有图片，或者降级处理），直接执行
    // 如果有文本，会在 getAsString 回调中执行
    if ((items && items.length > 0 && !hasText) || !items || items.length <= 0) {
      if (text || img) {
        this.handlePasteData({ text, img })
      } else {
        this.paste()
  ...
  // 处理粘贴的数据
  async handlePasteData(data) {
    let { text, img, unsupportedImageType } = data
    text = text || ''
    img = img || null
    const {
      errorHandler,
    ...
    try {
      ...
      if (text) {
        ...
        if (smmData) {
          const data = Array.isArray(smmData) ? smmData : [smmData]
  ...
  async paste() {
    ...
    if (!disabledClipboard && checkClipboardReadEnable()) {
      try {
        ...
        this.handlePasteData(res)
      } catch (error) {
        this.mindMap.opt.errorHandler(ERROR_TYPES.READ_CLIPBOARD_ERROR, error)
      }
    } else {
      // 尝试使用 execCommand 触发粘贴（降级支持）
      // 注意：大部分现代浏览器出于安全考虑不支持此命令，但在某些特定环境可能有效
      let isTriggered = false
      try {
        isTriggered = document.execCommand('paste')
      } catch (error) {
        console.error('execCommand paste error', error)
      }

      // 禁用剪贴板或不支持剪贴板时
      // 粘贴画布内的节点数据
      if (!isTriggered && this.beingCopyData) {
